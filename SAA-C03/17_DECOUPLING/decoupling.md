...
sqs는 소비자가 대기열에서 메시지를 요청해서 데이터를 가져오는 pull 모델입니다. 작업자나 소비자 수는 제한이 없습니다. 

관리된 서비스이므로 처리량을 프로비저닝할 필요가 없고 아주 빠르게 수백 수천 개의 메시지로 확장하며 fifo 대기열을 활성화합니다. 

--

sns는 데이터가 한번 sns에 전송되면 지속되지 않아요 데이터를 읽을 가능성이 있죠 게시 모델은 최대 10만 개의 주제로 확장 가능하며 역시 처리량을 프로비저닝하지 않아도 도비니다. 

kinesis는 두가지 소비 모드가 있습니다. 
소비자가 kinesis 로부터 데이터를 가져오는 표준 모드는 샤드당 2mb/s 속도를 지원합니다

반면 향상된 팬아웃 유형의 소비 매커니즘에서는 kinesis가 소비자에게 데이터를 푸시하며 샤드 하나에 소비자당 2mb/s 속도가 나옵니다. 처리량이 훨씬 높을 테니 스트림에서 더 많은 애플리케이션읽기가 가능합니다. 

데이터가 지속되기 때문에 다시 재생 가능합니다. 따라서 실시간 빅데이터 분석, etl 등에 활용합니다. 샤드 레벨에서 정할 수 있어 미리 kinesis 데이터 스트림마다 원하는 샤드 양을 지정해야 합니다. 샤드를 직접 확장해서 데티터가 언제 만료될지 정합니다. 

용량에는 두가지있습니다 프로비저닝 용량모드는 kinesis 데이터 스트림으로부터 원하는 사드양을 미리 지정하며 

온드맨드 용량 모드에서는 샤드 수가 kinesis 데이터 스트렘에 따라 자동으로 조정됩니다. 

# amazon mq

aws 독점 기술이기 대문에 클라우니 네이티브 서비스입니다. 

온프레미스에서 기존 애플리케이션을 실행하는 경우 개방형 프로토콜인
MQTT, AMQP, STOMP, WSS Openwire 등을 사용하면 됩니다

그리고 애플리케이션을 클라우드에 마이그레이션하는 경우에는
SQS, SNS 프로토콜 혹은 API를
사용하기 위해 애플리케이션을 다시 구축하고 싶지 않고
MQTT, AMQP 등과 같은 기존에 쓰던 프로토콜을 사용하고 싶을 수 있는데
이때 Amazon MQ를 쓰면 됩니다 상당히 간단한 서비스로

RabbitMQ와 ActiveMQ 두 가지 기술을 위한 관리형 메시지 브로커 서비스입니다

두가지는 온프레미스 기술로 개방형 프로토콜 엑세스를 제공합니다. 

amazon mq 를 이용하면 해당 브로커의 관리형 버전을 클라우드에서 사용합니다. 

amazon mq 는 무한 확장이 가능한 sqs나 sns 처럼 확장성이 크지 않습니다. 서버에서 실행되므로 서버 문제가 있을 수 있죠 고가용성을 위해 장애 조치와 함께 다중 az 설정을 실행합니다. 

*Amazon mq의 고 가용성은 예를 들어서 두영역에 각각 활성, 대기 상태인 mq 브로커를 추가합니다. 장애 조치 실행을 위한 백엔드 스토리지에 efs도 정의합니다. 장애 조치가 일어날 때마다 대기 상태 영역 역시 efs에 마운트되므로 첫번 째 활성 대기열과 동일한 데이터를 가질 수 있습니다.* 
